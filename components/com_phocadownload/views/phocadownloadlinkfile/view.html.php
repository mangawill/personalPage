...
class PhocaDownloadViewPhocaDownloadLinkFile extends JView
{
	...
	//	$filter_sectionid	= $app->getUserStateFromRequest( $this->_context.'.filter_sectionid','filter_sectionid',	-1,	'int');
		...
		if ($tmpl['type'] != 4) {
			$lists['catid'] = PhocaDownloadHelper::filterCategory($query, $catid, null, true);
		} else {
			$lists['catid'] = PhocaDownloadHelper::filterCategory($query, $catid, null, false);
		}
