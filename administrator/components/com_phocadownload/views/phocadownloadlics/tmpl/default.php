...
<form action="<?php echo JRoute::_('index.php?option=com_phocadownload&view=phocadownloadlics'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<tr>
		...
		<th width="13%">
					...
					if ($canOrder && $saveOrder) {
						echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'phocadownloadlics.saveorder');
					} ?>
					</th>
		<?php /*
		<th width="5%">
...
$linkEdit	= JRoute::_( 'index.php?option=com_phocadownload&task=phocadownloadlic.edit&id='.(int) $item->id );
...
<tr class="row<?php echo $i % 2; ?>">
	...
	<?php if ($item->checked_out) {
		echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'phocadownloadlics.', $canCheckin); 
	} 
	...
	<td class="center"><?php echo JHtml::_('jgrid.published', $item->published, $i, 'phocadownloadlics.', $canChange); ?></td>
	
<?php 
$cntx = 'phocadownloadlics';
echo '<td class="order">';
if ($canChange) {
	if ($saveOrder) {
		if ($listDirn == 'asc') {
