...
class PhocaDownloadRateHelper
{
	...
	public function renderRateFile($id, $displayRating, $small = 1, $refresh = false) {
		...
		//$amp 	= PhocaDownloadHelper::setQuestionmarkOrAmp($rating['urlvote']);
	...
	public function renderRateFileJS($small = 1) {
		...
		$url		  = 'index.php?option=com_phocadownload&view=ratingfilea&task=rate&format=json&'.JUtility::getToken().'=1';
		$urlRefresh		= 'index.php?option=com_phocadownload&view=ratingfilea&task=refreshrate&small='.$small.'&format=json&'.JUtility::getToken().'=1';
