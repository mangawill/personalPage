...
class PhocaDownloadFileUpload
{
	public function realMultipleUpload( $frontEnd = 0) {
		...
		$path	= PhocaDownloadHelper::getPathSet($manager);// we use viewback to get right path
	...
	function canUpload( $file, &$err, $manager = '', $frontEnd = 0, $chunkEnabled = 0, $realSize = 0) {
		...
		if ($frontEnd == 1) {
			$aft = $paramsC->get( 'allowed_file_types_upload', PhocaDownloadHelper::getDefaultAllowedMimeTypesUpload() );
			$dft = $paramsC->get( 'disallowed_file_types_upload', '' );
			$allowedMimeType 	= PhocaDownloadHelper::getMimeTypeString($aft);
			$disallowedMimeType = PhocaDownloadHelper::getMimeTypeString($dft);
		} else {
		
			$aft = $paramsC->get( 'allowed_file_types_download', PhocaDownloadHelper::getDefaultAllowedMimeTypesDownload() );
		...
		$html_tags = PhocaDownloadHelper::getHTMLTagsUpload();
	...
	function renderCreateFolder($sessName, $sessId, $currentFolder, $viewBack, $attribs = '') {
		...
		.'<legend>'.JText::_('COM_PHOCADOWNLOAD_FOLDER').'</legend>'."\n"
		.'<div class="path">'
		.'<input class="inputbox" type="text" id="foldername" name="foldername"  />'
		.'<input class="update-folder" type="hidden" name="folderbase" id="folderbase" value="'.$currentFolder.'" />'
		.'<button type="submit">'. JText::_( 'COM_PHOCADOWNLOAD_CREATE_FOLDER' ).'</button>'
		.'</div>'."\n"
	    .'</fieldset>'."\n"
		.JHTML::_( 'form.token' )
		.'</form>';
