...
class PhocaDownloadFileUploadMultiple
{
	...
	static public function renderMultipleUploadLibraries() {
		...
		$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/jquery/jquery-1.4.2.min.js');
		if ($uploadMethod == 2) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/gears_init.js');
		}
		if ($uploadMethod == 5) {
			$document->addScript('http://bp.yahooapis.com/2.4.21/browserplus-min.js');
		}
		$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.min.js');
		if ($uploadMethod == 2) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.gears.min.js');
		}
		if ($uploadMethod == 3) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.silverlight.min.js');
		}
		if ($uploadMethod == 1) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.flash.min.js');
		}
		if ($uploadMethod == 5) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.browserplus.min.js');
		}
		if ($uploadMethod == 6) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.html4.min.js');
		}
		if ($uploadMethod == 4) {
			$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/plupload.html5.min.js');
		}
		$document->addScript(JURI::root(true).'/components/com_phocadownload/assets/plupload/jquery.plupload.queue.min.js');
		JHTML::stylesheet( 'components/com_phocadownload/assets/plupload/css/plupload.queue.css' );
	}
	
	static public function getMultipleUploadSizeFormat($size) {
		$readableSize = PhocaDownloadHelper::getFileSizeReadable($size, '%01.0f %s', 1);
	...
	public function renderMultipleUploadJS($frontEnd = 0, $chunkMethod = 0) {
		...
		$js.='					window.location = \''.$this->reload.'\' + \'&muuploaded=\' + Uploader.total.uploaded + \'&mufailed=\' + Uploader.total.failed;'."\n";
